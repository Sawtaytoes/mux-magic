...
export type SpecialFeature = {
  ...
  // Parent's label text (e.g. "3 Side-By-Side Comparisons") so the
  // user-facing category prompt can surface the DVDCompare hierarchy
  // when a child like "Airplane Models" needs disambiguating.
  parentText?: string
  timecode?: string
  text: string
...
export type PossibleName = {
  ...
  // top-level entries.
  parentName?: string
  // DVDCompare's parsed Plex-category for this entry and (for children)
  // for its parent. Forwarded into the Smart Match candidate so the
  // server can append the same `-trailer` / `-featurette` /
  // `-behindthescenes` suffix the main NSF flow appends via
  // `getSpecialFeatureFromTimecode` — without this, picking a Smart
  // Match suggestion renamed the file to the bare extra title with no
  // category tag, breaking Plex's local-asset matching.
  type?: SpecialFeatureType
  parentType?: SpecialFeatureType
}
...
// Canonical form used for duplicate detection in `dedupePossibleNames`.
// Smart-quote variants and `:` / ` - ` separator variants collapse to
// the same key so DVDCompare's two parser paths (untimed branch keeps
// smart quotes, timed branch strips them) don't surface as two
// dropdown entries for the same feature.
...
// Collapses duplicate entries from DVDCompare's two duplication sources
// (cross-disc repeats + smart-quote/normalize variants) so the Smart
// Match dropdown shows each unique feature once. Grouping is by
// `(normalizedName, parentName)`; when duplicates collapse, the timed
// variant beats the untimed copy so the dropdown can still display
// runtimes. Original ordering is preserved.
export const dedupePossibleNames = (
  ...
  const result = possibleNames.reduce<Acc>(
    ({ keys, items }, current) => {
      const key = `${normalizeForDedupe(current.name)} ${normalizeForDedupe(current.parentName ?? "")}`
      const existingIndex = keys[key]
      if (existingIndex === undefined) {
        return {
          keys: { ...keys, [key]: items.length },
      ...
      return {
        ...
        items: items.map((item, index) =>
          index === existingIndex ? current : item,
        ),
      }
    },
    { keys: {}, items: [] },
...
export const flattenExtrasAsPossibleNames = (
  ...
  extras.flatMap((extra) =>
    [
      {
        name: extra.text,
        timecode: extra.timecode,
        type: extra.type,
        parentType: extra.parentType,
      },
    ].concat(
      (extra.children ?? []).map((child) => ({
        ...
        type: child.type,
        // For nested children DVDCompare's parser propagates the parent
        // extra's type into the child's `parentType` slot, so re-use
        // the child's own `parentType` here rather than the parent
        // SpecialFeature's `type` directly.
