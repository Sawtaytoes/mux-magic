// Per-file chapter renumber: extract the chapter XML, rewrite the
// `Chapter NN` names so they run 1..N, and remux the file with the
// rewritten XML. Metadata-only — timecodes, UIDs, languages and flags
// come through verbatim.
//
// This lives in tools/ rather than inside the `renumberChapters`
// app-command because two commands need it: `renumberChapters` walks a
// directory, and `splitChapters` runs it on each part it just produced
// (a split part inherits the play-all file's chapter numbers, so part 2
// starts at `Chapter 04` instead of `Chapter 01`).
...
import { aclSafeCopyFile, logError } from "@mux-magic/tools"
import {
  catchError,
  defer,
  EMPTY,
...
export const renumberChaptersInFile = ({
  filePath,
  isPaddingChapterNumbers,
}: {
  filePath: string
