...
export const getTextContentWithoutChildren = (
  ...
  Array.from(element.childNodes)
    .filter(
      ({ nodeType }) =>
        nodeType === new JSDOM().window.Node.TEXT_NODE,
    )
...
export const getReasonFromMovieTextDomSnippet = (
  ...
  const textContentWithoutChildren =
    getTextContentWithoutChildren(
      jsdomSnippet.window.document.body,
    )
...
export const getReasonsFromDomSnippet = (
  ...
  Array.from(
    jsdomSnippet.window.document.querySelectorAll<HTMLSpanElement>(
      "span",
    ) || [],
  )
    .map((element) =>
    ...
    .filter(Boolean)
    .concat(getReasonFromMovieTextDomSnippet(jsdomSnippet))
    .filter(Boolean)
    .concat(
      Array.from(
        jsdomSnippet.window.document.querySelectorAll<HTMLAnchorElement>(
          "[href]",
        ) || [],
      )
        .filter((element) =>
...
export const getSectionTitleFromDomSnippet = (
  ...
  (
    jsdomSnippet.window.document.querySelector(
...
export const getMovieDataFromDomSnippet = (
  ...
  const fakeElement =
    jsdomSnippet.window.document.createElement("div")
...
export const parseDomSnippetTextContent = (
  ...
  ...getMovieDataFromDomSnippet(jsdomSnippet),
  reasons: getReasonsFromDomSnippet(jsdomSnippet),
  sectionTitle: getSectionTitleFromDomSnippet(jsdomSnippet),
})
...
export const processUhdDiscForumPost = (
  ...
  formPostContent
    .split("<br>")
    .filter(Boolean)
    .map((htmlSection) => new JSDOM(htmlSection))
    .map((jsdomSnippet) =>
      parseDomSnippetTextContent(jsdomSnippet),
    )
    .filter(
    ...
    .reduce(
      ...
      [] satisfies UhdDiscForumPostGroup[] as UhdDiscForumPostGroup[],
    )
