...
Answering the TVDB prompt for season 1 against such a folder renamed the first 28
specials into `s01e01`…`s01e28` — a pilot became "The Flintstone Flyer" — while the
real season 1 files kept their original names. Two files then claimed the same episode
...
download; 25 were still byte-identical to their originals, which is the only reason
the mapping could be proven rather than guessed.

The owner's correction was explicit: *"This is something we fixed for naming anime
