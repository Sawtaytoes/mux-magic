...
`Bunnies/Family`, the pool this tool exists to manage, is `acltype=nfsv4` with
`aclmode=restricted`. `fs.cp` copies each file with libuv's `fs.copyFile`, whose
post-copy `fchmod` returns **EPERM** under restricted aclmode even when the mode is
unchanged. `fs.cp` exposes no hook to absorb that, so the whole recursive copy fails
...
was written. It does not work on this pool.
...
Reproduced directly inside the mux-magic container against `Bunnies/Family`:

- `fs.writeFileSync` into the destination directory — OK
- `fs.copyFileSync` within the same directory — **EPERM**
- `fs.copyFileSync` to `/tmp` (a different filesystem) — OK
- Python's `shutil.copyfile`, which does not copy permissions — OK

The owner's correction, 2026-09-16: *"This should've been solved already. We have code
in there to ensure the `chmod` gets caught and the code keeps working after that occurs.
If it's not fixed, we need to fix it. It was a long-standing issue in the past."*

`packages/tools/src/aclSafeCopyFolder.test.ts` covers the EPERM-after-complete-write
path over a whole tree.
