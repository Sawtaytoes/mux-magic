...
1. ~~**Unsupported VIDEO codec (HEVC/H.265, AV1) is out of scope → worker 2f.**~~ **UPDATED 2026-06-30 — now IMPLEMENTED, no longer out of scope.** Non-H.264 source video (VC-1 Blu-ray, MPEG-2 DVD, HEVC, AV1, VP9) is **re-encoded to H.264** so it plays in any browser. `fetchStreamInfo` sets `isVideoReencodeNeeded` when the source MediaInfo `Format` ≠ `AVC`; `buildFfmpegArgs` then emits `libx264` (CPU, `ultrafast`/`zerolatency`) or, when a usable NVIDIA encoder is detected once at startup (`detectNvencSupport.ts` runs a tiny probe encode — `-encoders` can't be trusted), `-hwaccel cuda -hwaccel_output_format cuda` + `h264_nvenc` (GPU decode via `vc1_cuvid`/`mpeg2_cuvid` → NVENC). **One image runs on GPU and non-GPU hosts** — no GPU falls back to libx264; `FFMPEG_DISABLE_NVENC=true` forces CPU. Output is H.264 High@L4.1 (`avc1.640029`) either path, advertised via `X-Video-Codec`. This **subsumes worker 2f's** GPU-reencode-endpoint idea (the NVENC auto-detect is that path, in-process). **DO NOT** revert the re-encode branch back to a `-c:v copy`-only "browser handles it" assumption — VC-1/MPEG-2 are the *primary* disc-rip codecs and were the whole point of this feature.
   - **No-audio files** are also handled: `buildFfmpegArgs` omits the audio map/encode when `plan.hasAudio` is false (`HEAD` reports `X-Has-Audio`). A `-map 0:a:0` against a video-only source (e.g. a logo bumper) makes ffmpeg exit before producing output — the original 0:00 stall. DO NOT reinstate an unconditional audio map.
