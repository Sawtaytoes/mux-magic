...
- **O1 — NOT a bug (initial finding retracted).** The `deleteFilesByExtension` default recursion depth `2 → 1` is an **intentional codebase-wide convention**, not drift: the handler matches its own schema doc ("0 = default depth of 1", `schemas.ts:639`) and every recursive command mirrors it (`convertLosslessToFlac`, `modifySubtitleMetadata`, `getSubtitleMetadata`). Now recorded as [a decision](../decisions/2026-05-20-default-recursion-depth-is-1.md). **DO NOT restore `|| 2`** from the v1.0.0 diff. (Caught by cross-checking two sweep agents — the file-org pass flagged it before the schema docs were read; the subtitle pass confirmed it's the documented convention.)
...
- **Default recursion depth of 1** (v1.0.0 used 2) across `deleteFilesByExtension` / `convertLosslessToFlac` / `modifySubtitleMetadata` / `getSubtitleMetadata` — an intentional convention, code matches schema docs. See [the decision](../decisions/2026-05-20-default-recursion-depth-is-1.md). Don't "restore" depth 2 from a v1 diff.
