...
Add a new app-command — `convertWavToFlac` — that walks a music directory for `.wav` files and encodes each one to FLAC via `ffmpeg -c:a flac`, **strictly lossless** (channels, bit depth, sample rate, and metadata are preserved). The FLAC is written **in-place** alongside the source WAV (same directory, same basename, `.flac` extension), and the source `.wav` is optionally deleted on success via an opt-in `isSourceDeleted` flag.
...
FFmpeg's FLAC encoder preserves the input's channel layout, sample rate, and bit depth **by default**. A 24-bit / 96 kHz / 5.1 WAV becomes a 24-bit / 96 kHz / 5.1 FLAC with no resampling and no downmix. The way to *keep* it lossless is to deliberately **not** add any of the following ffmpeg flags:
...
1. Compute destination as `<sourceDir>/<sourceBasename>.flac` (no folder prefix).
2. Invoke `runFfmpeg` with args `["-c:a", "flac", "-map_metadata", "0"]`, `inputFilePaths: [filePath]`, `outputFilePath`. No `mkdir` needed since the parent directory already exists (we're writing to the WAV's own directory).
...
type ConvertWavToFlacProps = {
  isRecursive: boolean
  isSourceDeleted?: boolean      // default false; deletes the .wav after a successful encode
  sourcePath: string
}
```

Export `convertWavToFlacDefaultProps` alongside the cli-spawn-op (same pattern as `convertFlacToPcmAudioDefaultProps`): `{ isSourceDeleted: false }`.

### Wiring (six surfaces)

1. **App-command:** [packages/core/src/app-commands/convertWavToFlac.ts](../../packages/core/src/app-commands/convertWavToFlac.ts) — new file.
2. **Cli-spawn-op:** [packages/core/src/cli-spawn-operations/convertWavToFlac.ts](../../packages/core/src/cli-spawn-operations/convertWavToFlac.ts) — clones the structure of `convertFlacToPcmAudio.ts` but writes in-place and (optionally) unlinks the source.
3. **WAV filter:** [packages/core/src/tools/filterIsWavFile.ts](../../packages/core/src/tools/filterIsWavFile.ts) — sibling of `filterIsAudioFile.ts`; the existing helper is too broad (matches `.mp3`, `.mkv`, etc).
4. **Schema:** [packages/api/src/api/schemas.ts](../../packages/api/src/api/schemas.ts) — `convertWavToFlacRequestSchema` (`sourcePath` path, `isRecursive` boolean, `isSourceDeleted` optional boolean).
...
8. **CLI wrapper:** [packages/cli/src/cli-commands/convertWavToFlacCommand.ts](../../packages/cli/src/cli-commands/convertWavToFlacCommand.ts) — mirrors [replaceFlacWithPcmAudioCommand.ts](../../packages/cli/src/cli-commands/replaceFlacWithPcmAudioCommand.ts).
9. **Fake-data scenario:** [packages/api/src/fake-data/scenarios/convertWavToFlac.ts](../../packages/api/src/fake-data/scenarios/convertWavToFlac.ts) — clone of `replaceFlacWithPcmAudio.ts` scenario but with `.wav → .flac` extensions; register in [packages/api/src/fake-data/index.ts](../../packages/api/src/fake-data/index.ts).

## TDD steps

1. **Failing unit test** — `convertWavToFlac.test.ts` (app-command) with `runFfmpeg` and `unlink` mocked. Cover:
   - One `.wav` in `sourcePath` → one `runFfmpeg` call.
   - The spawn args contain `-c:a flac` and `-map_metadata 0`.
   - The spawn args **do not** contain `-ar`, `-ac`, or `-sample_fmt` (lossless guard).
   - The output path equals the input path with `.wav` → `.flac` (in-place, same dir).
   - `.mp3` and `.mkv` siblings are ignored.
   - `isRecursive: true` descends one level.
   - `isSourceDeleted: false` (or omitted) → `unlink` is **not** called.
   - `isSourceDeleted: true` + successful ffmpeg → `unlink` called once with the original `.wav` path.
...
- [packages/core/src/app-commands/convertWavToFlac.ts](../../packages/core/src/app-commands/convertWavToFlac.ts)
- [packages/core/src/app-commands/convertWavToFlac.test.ts](../../packages/core/src/app-commands/convertWavToFlac.test.ts)
- [packages/core/src/cli-spawn-operations/convertWavToFlac.ts](../../packages/core/src/cli-spawn-operations/convertWavToFlac.ts)
- [packages/core/src/tools/filterIsWavFile.ts](../../packages/core/src/tools/filterIsWavFile.ts)
- [packages/api/src/fake-data/scenarios/convertWavToFlac.ts](../../packages/api/src/fake-data/scenarios/convertWavToFlac.ts)
...
- [packages/api/src/api/schemas.ts](../../packages/api/src/api/schemas.ts) — `convertWavToFlacRequestSchema`
